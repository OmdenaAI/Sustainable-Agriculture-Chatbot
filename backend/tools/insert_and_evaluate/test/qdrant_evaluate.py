...
class Evaluator:
    def __init__(self, extract_kw=False, reranking=True, qdrant_limit=5):
        logging.basicConfig(
        level=logging.INFO,
        ...
        self.qdrant_model = SentenceTransformer(self.qdrant_config["embedding_model"])
        self.scorer = BERTScorer(lang="en")

        self.config_path = "tools/insert_db/config/config.yaml"

        self.qa_json_path = "tools/insert_db/data/generated_questions_farmer_persona.jsonl"

        self.qdrant_url = self.config["qdrant_url"]
        self.qdrant_collection = "ws1-test"
    ...
    def evaluate_results_with_context(self, enhanced_results: List[Dict[str, Any]], ground_truth: str, ground_truth_chunk_id: str) -> Dict[str, Any]:
    ...
    def evaluate_qa_pair(self, qa_pair):
        ...
        limit = 20 if self.reranking else self.limit
...
if __name__ == "__main__":
    ...
    evaluator = Evaluator(reranking=True)
